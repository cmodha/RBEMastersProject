...
%   [Dose       A/B      Time       regrowth rate     phago rate]
N = [6           6         6                6           6];
    ...
    for k = 1:N(2) %New A/B Ratio
        ...
        for l = 1:N(3) %Number of days post RT
            for m = 1:N(4)  % regrowth rate
                for o = 1:N(5); %phagocytosis rate
                    cf = cellFraction(LQM,resultGUI,days(l),regrowth(m),phago(o)); %Calculating the new cell fraction given the regrowth rate and the time post RT
                    % Calculating plots
                    ...
                    resultAll.parameters(n,5) = phago(o);
                    disp(n);
                    n=n+1;
                end
            end
